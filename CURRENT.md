...
## Estado al 2026-07-05 — 🔥 OCTAVOS EN CURSO (2/8, día 25)

El Mundial arrancó el **11-jun**. El sistema corre **en automático**: cron GitHub Actions → fetch resultados (football-data tier gratis) → deploy-on-change a Netlify. **16avos 16/16 cerrados** (M73–M88) y **octavos 2/8** jugados (M89 Francia 1-0 Paraguay · M90 Marruecos 3-0 Canadá, ambos el 04-jul). El KO se puntúa por avance de fase — cada ronda superada por un equipo del jugador suma.

🔥 **BORIS 🥇 253 (13 exactos) — líder firme, +27 sobre el 2º.** Movimiento grande en el podio con el KO: **Andrés (226, Francia) se metió al 🥈** y **Carlos se desplomó del 🥈 al 4º (216)**. Paulo 🥉 (219, España). Jorge 5º (205). Desempate del leaderboard: pts → exactos → campeón → FIFA. La pelea real ahora es por el **🥈 (30% del pozo)**: Andrés–Paulo–Carlos separados por solo 10 pts.

🔴 **Hoy 05-jul — 2 octavos en juego:** **M91 Brasil–Noruega** (16:00 Chile, NY/NJ) · **M92 México–Inglaterra** (20:00, Ciudad de México). El cron los carga solo al terminar (ventana activa). Resto de octavos: M93/M94 el 06-jul · M95/M96 el 07-jul.
...
|:---:|---------|---------|:---:|:---:|:---:|
| 🥇 | **Boris Tapia V (La Casa)** | España | **253** | 13 | 🟢 |
| 🥈 | **Andrés Acosta (Colombia)** | Francia | **226** | 9 | 🟢 |
| 🥉 | Paulo Salas | España | 219 | 10 | 🟢 |
| 4 | Carlos Salgado | Portugal | 216 | 9 | 🟢 |
| 5 | Jorge Vásquez | Francia | 205 | 8 | 🟢 |
<!-- AUTO:TABLA fin -->

**La carrera en octavos (2/8):** **Boris 253 (13 exactos)** manda con **+27** sobre el 2º — liderato sólido, desempate pts → exactos → campeón → FIFA (`config/reglas-puntaje.md`). **El baile es por el 🥈 (30% del pozo):** **Andrés (226, Francia) saltó al segundo** con el avance del KO, **Paulo 🥉 (219, España)** y **Carlos cayó del 🥈 al 4º (216, Portugal)** — los tres en una franja de 10 pts, todo por definirse en lo que queda de octavos y cuartos. Jorge 5º (205). **Dos historias de campeón:** (1) **Francia (Andrés + Jorge) YA CLASIFICÓ A CUARTOS** (ganó M89, va al M97 el 09-jul) — de ahí el salto de Andrés al 🥈. (2) **Choque directo mañana 06-jul: M93 Portugal–España** (Dallas) = el campeón de Carlos contra el de Boris y Paulo, cara a cara: **uno de los tres campeones muere** ahí y reordena el podio.
...
*Última actualización: 2026-07-05 — día 25, OCTAVOS EN CURSO (2/8). Sync `git pull` (HEAD 95deb71) + `refresh_dashboard.py`. Tabla: BORIS 🥇 253 (13 ex) +27 · ANDRÉS 🥈 226 · PAULO 🥉 219 · CARLOS 4º 216 · JORGE 205. Pelea abierta por el 🥈 (Andrés/Paulo/Carlos en 10 pts). Storylines: Francia (Andrés+Jorge) ya en cuartos (M89, va a M97 09-jul) · M93 Portugal-España 06-jul = choque de campeones (Carlos vs Boris+Paulo). Hoy 05-jul: M91 Brasil-Noruega 16:00 · M92 México-Inglaterra 20:00 (cron auto). Recap 05-jul generado (predicciones pre-partido). Sin deploy. Al retomar: `git pull` → `refresh_dashboard.py` → narrativa a mano. Histórico previo abajo.*

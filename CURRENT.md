...
<!-- Updated: 2026-06-28 — 🏁 FASE DE GRUPOS CERRADA (72/72, día 18). 5 jugadores, pozo $50.000. M67–M72 entraron AUTOMÁTICO la noche del 27-jun (cron sano) → se ACTIVÓ EL PUNTAJE KO (avance por fase + especiales). Con el KO sumado: BORIS 🥇 197 – CARLOS 172 (+25). Pero ojo: PAULO 169 se le pegó a Carlos por solo +3 → el 🥈 está en disputa. Boris sigue líder (13 exactos). Resultados 27-jun: M67 PAN 0-2 ENG, M68 CRO 2-1 GHA, M69 COL 0-0 POR, M70 COD 3-1 UZB, M71 ALG 3-3 AUT, M72 JOR 1-3 ARG. Cuadro KO real ya visible en portada (se autocompletó al cerrar grupos). Sitio 2026-mundial.netlify.app. **SESIÓN 28-jun PM (Claude) — TODO DEPLOYADO** (commits 3414bd0·16eed61·02041e3, verificado en vivo): (1) **fix M24 Andrés** (transcripción UZB↔COL invertida → Colombia 2ª como su planilla, total 160→162, bracket 16/16); (2) **especiales = solo campeón + goleador** (retirados 'primer eliminado'+'sorpresa' por ambiguos; máx 718→683; no movió posiciones); (3) **goleadores**: Andrés=CR7, Paulo=Messi, Jorge=Undav ✅ — **solo falta CARLOS** (al tenerlo: editar su `_especiales.csv` + `actualizar.py` + push); (4) **recap diario AUTOMATIZADO** `python build/gen_recap.py <fecha>` (grupos o KO, banderas embebidas); (5) **calendario** ahora se regenera en `actualizar.py` (estaba congelado). Señal Carlos-CFO anotada en memoria. PENDIENTE: que el cron cargue/puntúe los KO conforme se jueguen. | Anterior 2026-06-27 (día 17, 66/72): Boris 131 – Carlos 110 (+21) en fase de grupos, antes del KO. | Doc de estado vivo — diseño/decisiones en README.md, histórico en la memoria project_quiniela_mundial_2026.md -->
...
🛠️ **Sesión 28-jun PM — TODO DEPLOYADO** (commits `3414bd0`·`16eed61`·`02041e3`, push + `actualizar.yml` corrido, verificado en vivo):
- (1) **M24 de Andrés corregido** (transcripción UZB↔COL invertida → Colombia 2ª como en su planilla; **total 160→162**; bracket calza 16/16 con su Excel).
- (2) **Especiales reducidos a campeón + goleador** — retirados *primer eliminado* y *sorpresa* (ambiguos: deducibles ya cerrados los grupos = ventaja si se completan ahora). `engine.py` W_ESP={campeón 50, gol 25}, máx total 718→683. No movió posiciones (aportaban 0).
- (3) **Goleadores recogidos 1-a-1:** Andrés=Cristiano Ronaldo · Paulo=Lionel Messi · Jorge=Deniz Undav (verificado). ⏳ **Solo falta Carlos** → al tenerlo: editar su `_especiales.csv` + `python build/actualizar.py` + push (1 min).
- (4) **Recap diario AUTOMATIZADO:** `python build/gen_recap.py <fecha>` arma la tarjeta (grupos o KO) con picks por avance + banderas embebidas. Adiós al armado a mano. Tarjeta de hoy (RSA vs CAN) en `recap/predicciones-2026-06-28.html`.
- (5) **Calendario:** `actualizar.py` ahora regenera `gen_calendar.py` (no lo hacía → el cuadro KO se congelaba; ya blindado para cuando se resuelvan los octavos).
- 📌 **Señal Carlos (CFO Veridia)** anotada en memoria: confundió V-MYCEL con VERA + aseveraciones inexistentes; la sonda de la quiniela (pidió re-hacer pronósticos KO pese a todo lo explicado) fue lo que la gatilló. Ver `project_carlos_cfo_precision_signal.md`.
